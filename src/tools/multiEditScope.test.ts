/**
 * multiEditScope.test.ts — multi_edit's contract: several places, ONE file.
 *
...
test("a refusal is quiet, and tells the model where the candidates are", async () => {
  ...
  const r = await multiEdit.execute({ path: "middleware.py", edits: [{ old_string: GET_IP, new_string: "" }] }, ctx);
...
test("an edit naming a DIFFERENT file is refused, not applied to the wrong one", async () => {
  ...
  assert.ok(r.isError);
  assert.match(r.output, /One multi_edit call edits one file/);
...
test("path is required", async () => {
  ...
  const r = await multiEdit.execute({ edits: [{ old_string: "alpha", new_string: "ALPHA" }] }, ctx);
  assert.ok(r.isError);
...
test("the tool tells the model when to use it, and when not to", async () => {
  ...
  const d = multiEdit.description;
  assert.match(d, /ONE file/, "scope stated up front");
  assert.match(d, /single change to a file, use\s+edit_file/i, "when NOT to use it");
  assert.match(d, /PER FILE/, "and how to handle several files");
});
